...
final class SignInViewController: UIViewController {
    ...
    private let titleLabel = AuthUI.titleLabel("Sign in")
    private let subtitleLabel = AuthUI.subtitleLabel("Welcome back")
    ...
    private let forgotButton: UIButton = {
        ...
        button.setTitleColor(.mcAccent, for: .normal)
        button.titleLabel?.font = .poppins(.medium, size: 13)
        return button
    }()

    private let nextButton = MCButton()
    private let signUpLink = AuthUI.linkButton(prefix: "New member? ", action: "Sign up")
    ...
    private func setupIdentifiers() {
        emailField.textField.accessibilityIdentifier = "signin_email_field"
        passwordField.textField.accessibilityIdentifier = "signin_password_field"
        nextButton.accessibilityIdentifier = "signin_next_button"
        signUpLink.accessibilityIdentifier = "signin_signup_link"
    }

    private func setupLayout() {
        [titleLabel, subtitleLabel, emailField, passwordField, forgotButton,
         errorLabel, nextButton, signUpLink, spinner].forEach { view.addSubview($0) }
        ...
        NSLayoutConstraint.activate([
            ...
            forgotButton.trailingAnchor.constraint(equalTo: titleLabel.trailingAnchor),

            errorLabel.topAnchor.constraint(equalTo: forgotButton.bottomAnchor, constant: 12),
            ...
            signUpLink.bottomAnchor.constraint(equalTo: safe.bottomAnchor, constant: -32)
        ])
    }
    ...
    private func setupActions() {
        ...
        signUpLink.addTarget(self, action: #selector(didTapSignUp), for: .touchUpInside)
    }
    ...
    @objc private func didTapForgot() { viewModel.goToForgotPassword() }
    @objc private func didTapNext() { view.endEditing(true); viewModel.signIn() }
    @objc private func didTapSignUp() { viewModel.goToSignUp() }
}
